...
def test_homepage_is_displaying(client):
    ...
    assert ('<meta name=description content="Cafébabel is the first participatory '
            'magazine made by and for young Europeans. Join the party!">'
            in response)
...
def test_social_networks_fallback_on_english_accounts(client):
    response = client.get('/en/')
    assert '<a href=https://www.instagram.com/cafebabelmagazine/' in response
    response = client.get('/fr/')
    assert '<a href=https://www.instagram.com/cafebabelmagazine/' in response
